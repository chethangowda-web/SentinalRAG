...
async def list_traces(
    ...
    traces = await trace_service.list_traces(db, skip=skip, limit=limit, user_id=current_user.id)
    total = await trace_service.count_traces(db, user_id=current_user.id)
    return {
        "total": total,
        "skip": skip,
        "limit": limit,
...
async def get_trace(
    ...
    if not trace or trace.user_id != current_user.id:
...
async def export_traces_json(
    ...
    traces = await trace_service.list_traces(db, skip=0, limit=10000, user_id=current_user.id)
